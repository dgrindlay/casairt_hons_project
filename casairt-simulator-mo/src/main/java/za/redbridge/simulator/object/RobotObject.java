...
public class RobotObject extends PhysicalObject {
    ...
    public void step(SimState sim) {
        ...
        // for(List<Double> reading : readings){
...
//        consumeEnergy();
    ...
    public void setBoundToResource(boolean value) {
        this.isBoundToResource = value;
    }
    ...
    //function to change the energy level of robot
    public void consumeEnergy()
    {
        //total energy used by sensors
        float sensor_consumption = 0;

        //adds the energy used by all the sensors
        for(AgentSensor sensor : phenotype.getSensors())
        {
            sensor_consumption += sensor.getConsumption();
        }
        ...
        if(wheelDrives == null){
            ...
            energy_level -= (sensor_consumption + (motor_consumption*consumption_modifier));
        }
    }

    //modifier to increase consumption if pushing resources
